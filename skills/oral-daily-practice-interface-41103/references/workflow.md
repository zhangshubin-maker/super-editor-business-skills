...
2. 应用样章；若当前 Bridge 只声明但不能实际执行页面级 phone 画布切换，则先建立并保存回读一个 phone 375 成熟基线页，后续只用 `editor_duplicate_slide` 复制该基线结构，不借助浏览器改画布；
...
使用成熟基线页复制时，复制后先回读确认页面仍为 phone 375、末尾 20px 灰色空区块与八个独立词项仍在，且数字模块为零；随后才允许绑定当前目录的九个模块。基线页尚承担复制模板职责时不得提前写入 provenance；应先完成全部页面复制，再逐页写入各自 provenance，防止来源身份串页。
