---
name: listening-not-hard-interface-41097
description: "使用 super-editor-control 按已确认的语义规则制作“小学英语听力不难专题训练”界面型教辅。用户要求从完整原课件生成或微调这类书、使用默认样章 41097 或其特殊目录变体、复制文本/图片/数字模块、复用或删除区块、调整布局与大纲，或按已验证规则批量处理书单时使用；以语义理解和原子编辑能力执行。"
---
...
3. 默认选样章 `41097`；仅当一个最高优先级 `templates.variants.when` 语义命中时使用该变体。
4. 按 `order` 执行 4 条规则。用语义角色、基数、结构与锚点共同匹配；sourceId/template
