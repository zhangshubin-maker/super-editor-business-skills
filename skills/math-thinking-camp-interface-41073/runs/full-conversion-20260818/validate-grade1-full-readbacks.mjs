...
const index = JSON.parse(fs.readFileSync(path.join(runDir, 'grade1-full-provenance-index.json'), 'utf8'))
const receipts = []
...
for (const item of index) {
  ...
  receipts.push({ sourceId: item.sourceId, targetSlideId: item.targetSlideId, receiptPath: out, valid: receipt.valid !== false, receipt })
}

fs.writeFileSync(path.join(runDir, 'grade1-full-readback-index.json'), `${JSON.stringify(receipts, null, 2)}\n`, 'utf8')
console.log(JSON.stringify({ validated: receipts.length, valid: receipts.filter((item) => item.valid).length, lastReceipt: receipts.at(-1)?.receiptPath }, null, 2))
